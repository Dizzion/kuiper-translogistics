...
export const TNGetByTN = async (
  ...
  const res = await fetch(
    `http://10.68.140.169:8090/api/collections/TrackingNumbers/records?filter=(TrackingNumber='${trackingNumber}')`,
    { cache: "no-store" }
  );
...
export const ContGetByContId = async (contId: string): Promise<RecordModel> => {
  const res = await fetch(
    `http://10.68.140.169:8090/api/collections/Containers/records?filter=(ContainerID='${contId}')`,
    { cache: "no-store" }
  );
